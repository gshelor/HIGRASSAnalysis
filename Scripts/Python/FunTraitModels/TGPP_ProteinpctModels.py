...
### splitting x (band reflectances) and y (ProteinPct, variable being modeled)
TGPP_ProteinPct_x = TGPP_FunBmass_Airborne_df.select(pl.selectors.starts_with("band"))
TGPP_ProteinPct_y = TGPP_FunBmass_Airborne_df.select("Protein_pct")

### separating x and y into respective training and testing datasets
# TGPP_ProteinPct_trainx, TGPP_ProteinPct_testx, TGPP_ProteinPct_trainy, TGPP_ProteinPct_testy = train_test_split(TGPP_ProteinPct_x, TGPP_ProteinPct_y, test_size = 0.3, random_state = 802)
...
for x in np.arange(1, 51):
    ...
    # temp_cv = cross_validate(temp_pls, X = TGPP_ProteinPct_trainx, y = TGPP_ProteinPct_trainy, cv = Shuffle, scoring = PLS_metrics, return_train_score = True, return_estimator = True, return_indices = True, n_jobs = 10)
    # temp_cv.keys()
    for i in np.arange(0, 50):
        ### separating x and y into respective training and testing datasets
        ## using the iteration of the for as the seed (adding it to 802 because that's my go-to seed, Vermont forever)
        TGPP_ProteinPct_trainx, TGPP_ProteinPct_testx, TGPP_ProteinPct_trainy, TGPP_ProteinPct_testy = train_test_split(TGPP_ProteinPct_x, TGPP_ProteinPct_y, test_size = 0.3, random_state = 802 + i)
        ### fit model
        random.seed(802)
        temp_pls_fit = temp_pls.fit(TGPP_ProteinPct_trainx, TGPP_ProteinPct_trainy)
        ### predict on bootstrapped test set
        random.seed(802)
        temp_preds = temp_pls.predict(TGPP_ProteinPct_testx)
        ### extracting relevant metrics (R2, RMSE)
        ## will be used to identify optimal number of components
        temp_rmse = np.sqrt(mean_squared_error(TGPP_ProteinPct_testy, temp_preds))
        boot_rmse_list.append(temp_rmse)
        boot_R2_list.append(temp_pls_fit.score(TGPP_ProteinPct_testx, TGPP_ProteinPct_testy))
...
TGPP_ProteinPct_x = TGPP_FunBmass_Airborne_df.select("Quadrat_name", pl.selectors.starts_with("band"))
TGPP_ProteinPct_y = TGPP_FunBmass_Airborne_df.select("Protein_pct")
...
for i in np.arange(0, 50):
        ...
        TestQuadrats.append(TGPP_ProteinPct_testx['Quadrat_name'])
        ### only keeping band reflectances for PLSR model
        TGPP_ProteinPct_trainx = TGPP_ProteinPct_trainx.select(pl.selectors.starts_with("band"))
        TGPP_ProteinPct_testx = TGPP_ProteinPct_testx.select(pl.selectors.starts_with("band"))
        ### fit model
        random.seed(802)
        OptimalPLS_fit = OptimalPLS.fit(TGPP_ProteinPct_trainx, TGPP_ProteinPct_trainy)
        ### predict on bootstrapped test set
        random.seed(802)
        temp_preds = OptimalPLS_fit.predict(TGPP_ProteinPct_testx)
        TestPreds.append(temp_preds)
        ### extracting relevant metrics (R2, RMSE)
        ## will be used to identify optimal number of components
        temp_rmse = np.sqrt(mean_squared_error(TGPP_ProteinPct_testy, temp_preds))
        boot_rmse_list.append(temp_rmse)
        boot_R2_list.append(OptimalPLS_fit.score(TGPP_ProteinPct_testx, TGPP_ProteinPct_testy))
        ...
        TGPP_ProteinPct_array = (TGPPAirborne_2025_rast * OptimalPLSCoefs[:, None, None]
            ).sum(dim = "band") + OptimalPLSIntercept
        
        ### Path to save the result
        output_file_path = os.path.join(data_dir, "FunctionalTraits/TGPP/Outputs/Rasters/", "TGPP2025ProteinPct_Pred" + str(i) + ".tif")
        ...
        TGPP_ProteinPct_array.rio.to_raster(output_file_path, dtype=np.float32)
...
TestPredAvg_df = TestPredAvg_df.join(TGPP_FunBmass_Airborne_df.select("Quadrat_name", "Protein_pct"), on = "Quadrat_name")
...
plt.title('Foliar Nitrogen (%) \n PLSR Predicted vs. Observed Values \n (Bootstrap Averaged, k = 50)', fontsize=16)
plt.xlabel('Observed TN (%)', fontsize = 12)
plt.ylabel('Mean Predicted TN (%)', fontsize = 12)
# plt.xlim(min_val - padding, max_val + padding)
# plt.ylim(min_val - padding, max_val + padding)
plt.legend()
# plt.tight_layout()
# plt.show()
plt.savefig("/home/gshelor/Documents/Schoolwork/OKSt/HIGRASS/Outputs/Figures/FunctionalTraits/TGPP/2025/PLSRConfidenceIntervals/ProteinPctSDPlot.png")
plt.close()
### reading in exported raster for plotting
# TGPP_ProteinPct_rast = rxr.open_rasterio(os.path.join(data_dir, "FunctionalTraits/TGPP/Outputs/Rasters/TGPP2025ProteinPct.tif"))

### trying to plot it
# ep.plot_bands(TGPP_ProteinPct_rast, cmap = 'viridis')
