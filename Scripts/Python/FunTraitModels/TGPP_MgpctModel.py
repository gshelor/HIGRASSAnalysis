...
### splitting x (band reflectances) and y (Mgpct, variable being modeled)
TGPP_Mgpct_x = TGPP_FunBmass_Airborne_df.select(pl.selectors.starts_with("band"))
TGPP_Mgpct_y = TGPP_FunBmass_Airborne_df.select("Mg_pct")

### separating x and y into respective training and testing datasets
# TGPP_Mgpct_trainx, TGPP_Mgpct_testx, TGPP_Mgpct_trainy, TGPP_Mgpct_testy = train_test_split(TGPP_Mgpct_x, TGPP_Mgpct_y, test_size = 0.3, random_state = 802)
...
for x in np.arange(1, 51):
    ...
    # temp_cv = cross_validate(temp_pls, X = TGPP_Mgpct_trainx, y = TGPP_Mgpct_trainy, cv = Shuffle, scoring = PLS_metrics, return_train_score = True, return_estimator = True, return_indices = True, n_jobs = 10)
    # temp_cv.keys()
    for i in np.arange(0, 50):
        ### separating x and y into respective training and testing datasets
        ## using the iteration of the for as the seed (adding it to 802 because that's my go-to seed, Vermont forever)
        TGPP_Mgpct_trainx, TGPP_Mgpct_testx, TGPP_Mgpct_trainy, TGPP_Mgpct_testy = train_test_split(TGPP_Mgpct_x, TGPP_Mgpct_y, test_size = 0.3, random_state = 802 + i)
        ### fit model
        random.seed(802)
        temp_pls_fit = temp_pls.fit(TGPP_Mgpct_trainx, TGPP_Mgpct_trainy)
        ### predict on bootstrapped test set
        random.seed(802)
        temp_preds = temp_pls.predict(TGPP_Mgpct_testx)
        ### extracting relevant metrics (R2, RMSE)
        ## will be used to identify optimal number of components
        temp_rmse = np.sqrt(mean_squared_error(TGPP_Mgpct_testy, temp_preds))
        boot_rmse_list.append(temp_rmse)
        boot_R2_list.append(temp_pls_fit.score(TGPP_Mgpct_testx, TGPP_Mgpct_testy))
...
TGPP_Mgpct_x = TGPP_FunBmass_Airborne_df.select("Quadrat_name", pl.selectors.starts_with("band"))
TGPP_Mgpct_y = TGPP_FunBmass_Airborne_df.select("Mg_pct")
...
for i in np.arange(0, 50):
        ...
        TestQuadrats.append(TGPP_Mgpct_testx['Quadrat_name'])
        ### only keeping band reflectances for PLSR model
        TGPP_Mgpct_trainx = TGPP_Mgpct_trainx.select(pl.selectors.starts_with("band"))
        TGPP_Mgpct_testx = TGPP_Mgpct_testx.select(pl.selectors.starts_with("band"))
        ### fit model
        random.seed(802)
        OptimalPLS_fit = OptimalPLS.fit(TGPP_Mgpct_trainx, TGPP_Mgpct_trainy)
        ### predict on bootstrapped test set
        random.seed(802)
        temp_preds = OptimalPLS_fit.predict(TGPP_Mgpct_testx)
        TestPreds.append(temp_preds)
        ### extracting relevant metrics (R2, RMSE)
        ## will be used to identify optimal number of components
        temp_rmse = np.sqrt(mean_squared_error(TGPP_Mgpct_testy, temp_preds))
        boot_rmse_list.append(temp_rmse)
        boot_R2_list.append(OptimalPLS_fit.score(TGPP_Mgpct_testx, TGPP_Mgpct_testy))
        ...
        TGPP_Mgpct_array = (TGPPAirborne_2025_rast * OptimalPLSCoefs[:, None, None]
            ).sum(dim = "band") + OptimalPLSIntercept
        
        ### Path to save the result
        output_file_path = os.path.join(data_dir, "FunctionalTraits/TGPP/Outputs/Rasters/Magnesium/", "TGPP2025Mgpct_Pred" + str(i) + ".tif")
        ...
        TGPP_Mgpct_array.rio.to_raster(output_file_path, dtype=np.float32)
...
TestPredAvg_df = TestPredAvg_df.join(TGPP_FunBmass_Airborne_df.select("Quadrat_name", "Mg_pct"), on = "Quadrat_name")
...
plt.title('Foliar Magnesium (%) \n PLSR Predicted vs. Observed Values \n (Bootstrap Averaged, k = 50)', fontsize=16)
plt.xlabel('Observed Mg (%)', fontsize = 12)
plt.ylabel('Mean Predicted Mg (%)', fontsize = 12)
# plt.xlim(min_val - padding, max_val + padding)
# plt.ylim(min_val - padding, max_val + padding)
plt.legend()
# plt.tight_layout()
# plt.show()
plt.savefig("/home/gshelor/Documents/Schoolwork/OKSt/HIGRASS/Outputs/Figures/FunctionalTraits/TGPP/2025/PLSRConfidenceIntervals/MgpctSDPlot.png")
plt.close()
...
# TGPP_Mgpct_rast = rxr.open_rasterio(os.path.join(data_dir, "FunctionalTraits/TGPP/Outputs/Rasters/TGPP2025Mgpct.tif"))

### trying to plot it
# ep.plot_bands(TGPP_Mgpct_rast, cmap = 'viridis')