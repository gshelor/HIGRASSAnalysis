...
### splitting x (band reflectances) and y (Capct, variable being modeled)
TGPP_Capct_x = TGPP_FunBmass_Airborne_df.select(pl.selectors.starts_with("band"))
TGPP_Capct_y = TGPP_FunBmass_Airborne_df.select("Ca_pct")

### separating x and y into respective training and testing datasets
# TGPP_Capct_trainx, TGPP_Capct_testx, TGPP_Capct_trainy, TGPP_Capct_testy = train_test_split(TGPP_Capct_x, TGPP_Capct_y, test_size = 0.3, random_state = 802)
...
for x in np.arange(1, 51):
    ...
    # temp_cv = cross_validate(temp_pls, X = TGPP_Capct_trainx, y = TGPP_Capct_trainy, cv = Shuffle, scoring = PLS_metrics, return_train_score = True, return_estimator = True, return_indices = True, n_jobs = 10)
    # temp_cv.keys()
    for i in np.arange(0, 50):
        ### separating x and y into respective training and testing datasets
        ## using the iteration of the for as the seed (adding it to 802 because that's my go-to seed, Vermont forever)
        TGPP_Capct_trainx, TGPP_Capct_testx, TGPP_Capct_trainy, TGPP_Capct_testy = train_test_split(TGPP_Capct_x, TGPP_Capct_y, test_size = 0.3, random_state = 802 + i)
        ### fit model
        random.seed(802)
        temp_pls_fit = temp_pls.fit(TGPP_Capct_trainx, TGPP_Capct_trainy)
        ### predict on bootstrapped test set
        random.seed(802)
        temp_preds = temp_pls.predict(TGPP_Capct_testx)
        ### extracting relevant metrics (R2, RMSE)
        ## will be used to identify optimal number of components
        temp_rmse = np.sqrt(mean_squared_error(TGPP_Capct_testy, temp_preds))
        boot_rmse_list.append(temp_rmse)
        boot_R2_list.append(temp_pls_fit.score(TGPP_Capct_testx, TGPP_Capct_testy))
...
TGPP_Capct_x = TGPP_FunBmass_Airborne_df.select("Quadrat_name", pl.selectors.starts_with("band"))
TGPP_Capct_y = TGPP_FunBmass_Airborne_df.select("Ca_pct")
...
for i in np.arange(0, 50):
        ...
        TestQuadrats.append(TGPP_Capct_testx['Quadrat_name'])
        ### only keeping band reflectances for PLSR model
        TGPP_Capct_trainx = TGPP_Capct_trainx.select(pl.selectors.starts_with("band"))
        TGPP_Capct_testx = TGPP_Capct_testx.select(pl.selectors.starts_with("band"))
        ### fit model
        random.seed(802)
        OptimalPLS_fit = OptimalPLS.fit(TGPP_Capct_trainx, TGPP_Capct_trainy)
        ### predict on bootstrapped test set
        random.seed(802)
        temp_preds = OptimalPLS_fit.predict(TGPP_Capct_testx)
        TestPreds.append(temp_preds)
        ### extracting relevant metrics (R2, RMSE)
        ## will be used to identify optimal number of components
        temp_rmse = np.sqrt(mean_squared_error(TGPP_Capct_testy, temp_preds))
        boot_rmse_list.append(temp_rmse)
        boot_R2_list.append(OptimalPLS_fit.score(TGPP_Capct_testx, TGPP_Capct_testy))
        ...
        TGPP_Capct_array = (TGPPAirborne_2025_rast * OptimalPLSCoefs[:, None, None]
            ).sum(dim = "band") + OptimalPLSIntercept
        
        ### Path to save the result
        output_file_path = os.path.join(data_dir, "FunctionalTraits/TGPP/Outputs/Rasters/Calcium/", "TGPP2025Capct_Pred" + str(i) + ".tif")
        ...
        TGPP_Capct_array.rio.to_raster(output_file_path, dtype=np.float32)
...
TestPredAvg_df = TestPredAvg_df.join(TGPP_FunBmass_Airborne_df.select("Quadrat_name", "Ca_pct"), on = "Quadrat_name")
...
# plt.tight_layout()
# plt.show()
plt.savefig("/home/gshelor/Documents/Schoolwork/OKSt/HIGRASS/Outputs/Figures/FunctionalTraits/TGPP/2025/PLSRConfidenceIntervals/CapctSDPlot.png")
plt.close()
...
# TGPP_Capct_rast = rxr.open_rasterio(os.path.join(data_dir, "FunctionalTraits/TGPP/Outputs/Rasters/TGPP2025Capct.tif"))

### trying to plot it
# ep.plot_bands(TGPP_Capct_rast, cmap = 'viridis')